...
tree.controller('queryNode', function($scope, treeRoot, addEvent) {
    ...
    $scope.root.search = function(query) {
        ...
        while (queue.length > 0) {
            ...
            // 读取当前结点data
            if (current.data.toLowerCase() == query) resultList.push(current); //找到了
            // 将当前结点的所有孩子节点入“待访问”队
    ...
    /**
     * 关键字查询
     * @param  {[type]} nodevalue [description]
     * @return {[type]}           [description]
     */
    $scope.query = function(nodevalue) {
        if (nodevalue) {
            var keyword = nodevalue.trim();
            // 执行搜索
            var resultList = $scope.root.search(keyword);
            console.log(resultList);
            // 处理搜索结果
            if (resultList.length == 0) {
                $scope.tip = false;
                $scope.msg = "没有查询到符合条件的结点！";
            } else {
                $scope.tip = false;
                $scope.msg = "查询到" + resultList.length + "个符合条件的结点";
 ...
 * 将treeNode作为服务定义
 */
tree.factory('TreeNode', function() {
        ...
        render: function(arrow, visibility, toHighlight, deHighlight) {
            ...
            if (arrow) {
                if (this.isLeaf()) { // 是个叶节点，设为空箭头
                    this.selfElement.getElementsByClassName("arrow")[0].className = "arrow arrow-empty";
...
tree.factory('treeRoot', function(TreeNode) {
 ...
 * 定义事件绑定工具服务
 */
tree.factory('addEvent', function() {
    // 跨浏览器兼容的工具函数
